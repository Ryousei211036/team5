...
HRESULT CModel::Init()
{
	...
	//頂点数の取得
	nNumVtx = m_Model.m_pMesh->GetNumVertices();

	//頂点フォーマットのサイズを取得
	sizeFVF = D3DXGetFVFVertexSize(m_Model.m_pMesh->GetFVF());

	//頂点バッファのロック
	m_Model.m_pMesh->LockVertexBuffer(D3DLOCK_READONLY, (void**)&pVtxBuff);
	...
	//頂点バッファのアンロック
	m_Model.m_pMesh->UnlockVertexBuffer();
...
void CModel::Uninit()
{
	...
	{
		//メッシュの破棄
		if (m_Model.m_pMesh != NULL)
		{
			m_Model.m_pMesh->Release();
			m_Model.m_pMesh = NULL;
		}

		//マテリアルの破棄
		if (m_Model.m_pBuffMat != NULL)
		{
			m_Model.m_pBuffMat->Release();
			m_Model.m_pBuffMat = NULL;
		}
	}
}
...
void CModel::Draw(D3DXMATRIX mtxworld)
{
	...
	// マテリアルデータへのポインタを取得
	pMat = (D3DXMATERIAL*)m_Model.m_pBuffMat->GetBufferPointer();

	for (int nCntMat = 0; nCntMat < (int)m_Model.m_nNumMat; nCntMat++)
	{
		// マテリアル情報の設定
		...
		// モデルパーツの描画
		m_Model.m_pMesh->DrawSubset(nCntMat);
	}
...
void CModel::DrawShadow()
{
	...
	for (int nCnt = 0; nCnt < (int)m_Model.m_nNumMat; nCnt++)
	{
		// マテリアル情報の設定
		...
		// モデルパーツの描画
		m_Model.m_pMesh->DrawSubset(nCnt);
	}
